...
class GhoulforallMod(loader.Module):
    ...
    async def example_callback_handler(self, app: Client, call: CallbackQuery):  # _callback_handler на конце функции чтобы обозначить что это каллбек-хендлер
        """Пример каллбека"""
        await self.bot.send_message(inline_message_id=call.inline_message_id, text='Я гуль')
        await sleep(2)
        a = 1000
        while a > 0:
          c = a - 7
          await self.bot.send_message(inline_message_id=call.inline_message_id, text=str(a) + " - 7 = " + str(c))
          a = c
          await sleep(0.1)
        await self.bot.send_message(inline_message_id=call.inline_message_id, text='l l let me die')
